...
class TestIconScoreEngine(unittest.TestCase):
    @patch(
        "iconservice.iconscore.icon_score_context_util.IconScoreContextUtil.validate_score_blacklist"
    )
    ...
    @patch(
        "iconservice.iconscore.icon_score_engine.IconScoreEngine._validate_score_blacklist"
    )
    ...
    @patch("iconservice.iconscore.icon_score_engine.IconScoreEngine._call")
    def test_query(
        self, mocked_score_engine_call, mocked_score_engine_validate_score_blacklist
    ):
    ...
    @patch(
        "iconservice.iconscore.icon_score_context_util.IconScoreContextUtil.get_icon_score"
    )
    @patch(
        "iconservice.iconscore.icon_score_engine.IconScoreEngine._validate_score_blacklist"
    )
    def test_get_score_api(
        self,
        ...
        self.assertRaises(
            ...
            IconScoreEngine.get_score_api,
            context,
            score_address,
        )
        mocked_icon_score_engine_validate_score_blacklist.assert_called_with(
    ...
    @patch(
        "iconservice.iconscore.icon_score_engine.IconScoreEngine._convert_score_params_by_annotations"
    )
    ...
    def test_call(
        ...
        primitive_params = {
            "address": str(create_address(AddressPrefix.EOA)),
            "integer": "0x10",
        }
        converted_params = {"address": create_address(AddressPrefix.EOA), "integer": 10}
        mocked_score_engine_convert_score_params_by_annotations.return_value = (
            converted_params
        )
        context.set_func_type_by_icon_score = Mock()
        setattr(
            score_object, ATTR_SCORE_CALL, Mock(side_effect=intercept_score_base_call)
        )

        # set method, and params, method cannot be None as pre-validate it
        data = {"method": "score_method", "params": primitive_params}
    ...
    def test_convert_score_params_by_annotations(self):
        ...
        converted_params = IconScoreEngine._convert_score_params_by_annotations(
            score_object, "test_method", primitive_params
        )

        validate_external_method = getattr(
            score_object, ATTR_SCORE_VALIDATATE_EXTERNAL_METHOD
        )
        ...
        not_matching_type_params = {"address": b"bytes_data", "integer": "string_data"}
        ...
        converted_params = IconScoreEngine._convert_score_params_by_annotations(
            score_object, "test_method", insufficient_params
        )
